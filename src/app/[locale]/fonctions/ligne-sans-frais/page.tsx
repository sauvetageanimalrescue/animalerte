...
import { IconPhone, IconCheck, IconShieldLock } from "@tabler/icons-react";
import { Link } from "@/i18n/navigation";

export default async function PageLigne({
  params,
}: PageProps<"/[locale]/fonctions/ligne-sans-frais">) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("pageLigne");
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <div className="mt-8 flex gap-4 rounded-2xl border border-brand/30 bg-brand-soft p-5">
        <IconPhone size={28} className="mt-0.5 shrink-0 text-brand" />
        <div>
      ...
      {/* Une exclusivité */}
      <div className="mt-8 flex gap-4 rounded-2xl border border-accent/30 bg-accent-soft p-5">
        <IconShieldLock size={28} className="mt-0.5 shrink-0 text-accent" />
        <div>
