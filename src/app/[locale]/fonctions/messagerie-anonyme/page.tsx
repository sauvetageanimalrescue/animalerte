...
export default async function PageMessagerie({
  params,
}: PageProps<"/[locale]/fonctions/messagerie-anonyme">) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("pageMessagerie");
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      {/* Un message, sans partager de numéro */}
      <div className="mt-8 flex gap-4 rounded-2xl border border-brand/30 bg-brand-soft p-5">
        <IconMessage size={28} className="mt-0.5 shrink-0 text-brand" />
        <div>
