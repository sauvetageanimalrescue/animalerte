...
export default async function PageAlerteSentinelle({
  params,
}: PageProps<"/[locale]/fonctions/alerte-sentinelle">) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("pageSentinelle");
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      {/* Des yeux là où votre animal a disparu */}
      <div className="mt-8 flex gap-4 rounded-2xl border border-brand/30 bg-brand-soft p-5">
        <IconBroadcast size={28} className="mt-0.5 shrink-0 text-brand" />
        <div>
      ...
      <div className="mt-10 flex gap-4 rounded-2xl border border-border bg-surface p-5">
        <IconUsers size={26} className="mt-0.5 shrink-0 text-brand" />
        <div>
          <h2 className="font-bold text-brand-dark">{t("reseauTitre")}</h2>
