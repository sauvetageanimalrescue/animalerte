...
import { IconHeartHandshake, IconHeart, IconCheck } from "@tabler/icons-react";
import { Link } from "@/i18n/navigation";

export default async function PageDiffusionPartenaire({
  params,
}: PageProps<"/[locale]/fonctions/diffusion-partenaire">) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("pagePartenaire");
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <div className="mt-8 flex gap-4 rounded-2xl border border-brand/30 bg-brand-soft p-5">
        <IconHeartHandshake size={28} className="mt-0.5 shrink-0 text-brand" />
        <div>
      ...
      {/* Une communauté qui a la cause à cœur */}
      <div className="mt-10 flex gap-4 rounded-2xl border border-border bg-surface p-5">
        <IconHeart size={26} className="mt-0.5 shrink-0 text-brand" />
        <div>
          <h2 className="font-bold text-brand-dark">{t("communauteTitre")}</h2>
