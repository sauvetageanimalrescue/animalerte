...
import { IconArrowBigUpLines, IconClock, IconCheck } from "@tabler/icons-react";
import { Link } from "@/i18n/navigation";

export default async function PageAnnoncePrioritaire({
  params,
}: PageProps<"/[locale]/fonctions/annonce-prioritaire">) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("pagePrioritaire");
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      {/* En tête, là où les gens regardent en premier */}
      <div className="mt-8 flex gap-4 rounded-2xl border border-brand/30 bg-brand-soft p-5">
        <IconArrowBigUpLines size={28} className="mt-0.5 shrink-0 text-brand" />
        <div>
      ...
      {/* Sept jours en vedette */}
      <div className="mt-10 flex gap-4 rounded-2xl border border-border bg-surface p-5">
        <IconClock size={26} className="mt-0.5 shrink-0 text-brand" />
        <div>
          <h2 className="font-bold text-brand-dark">{t("dureeTitre")}</h2>
