...
import { IconQrcode, IconCheck } from "@tabler/icons-react";
import { Link } from "@/i18n/navigation";

export default async function PageAffiche({
  params,
}: PageProps<"/[locale]/fonctions/affiche-intelligente">) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("pageAffiche");
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      {/* Le code QR ramène à la fiche */}
      <div className="mt-8 flex gap-4 rounded-2xl border border-brand/30 bg-brand-soft p-5">
        <IconQrcode size={28} className="mt-0.5 shrink-0 text-brand" />
        <div>
